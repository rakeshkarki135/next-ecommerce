...
const ProductList = async ({ categoryId,
    ...
    return (
        <div className="mt-12 flex gap-x-8 gap-y-16 justify-between flex-wrap">
            { (await res).items.map((product:products.Product)=>( 
                <Link 
                    href={"/"+product.slug} 
                    ...
                    <button className="rounded-2xl ring-1 ring-rakesh w-max py-2 px-4 text-xs hover:bg-rakesh hover:text-white">Add to Cart</button>
                </Link>
            ))}
